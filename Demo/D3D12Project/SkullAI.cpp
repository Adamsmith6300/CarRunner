...
POS SkullAI::CalcMove(Entity* target) {
	/*int decisionX = rand() % 5 + 1;

	POS displacement = {0.0f, 0.01f, 0.0f};

	if (target->GetPosition3f().x > parentSkull->GetPosition3f().x) {
		displacement.x += decisionX;
	}
	if (target->GetPosition3f().x < parentSkull->GetPosition3f().x) {
		displacement.x -= decisionX;
	}
	if (target->GetPosition3f().z < parentSkull->GetPosition3f().z) {
		displacement.z = target->GetPosition3f().z - parentSkull->GetPosition3f().z;
	}
	if (target->GetPosition3f().z > parentSkull->GetPosition3f().z) {
		displacement.z = target->GetPosition3f().z - parentSkull->GetPosition3f().z;
	}*/
